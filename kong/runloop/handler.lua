...
local function get_rebuild_timeout()
  if kong.configuration.database == "cassandra" then
    return kong.configuration.cassandra_timeout / 1000
  end

  if kong.configuration.database == "postgres" then
    return kong.configuration.pg_timeout / 1000
  end
...
do
  ...
  rebuild = function(name, callback, version, semaphore, timeout)
    ...
    local ok, err = acquire_semaphore(semaphore, timeout)
    if not ok then
      return nil, err
    end
