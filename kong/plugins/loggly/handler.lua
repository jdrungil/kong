...
function LogglyLogHandler:access(conf)
  local ok, err = serializers.load_serializer(conf.serializer or "default")
  if not ok then
    ngx.log(ngx.ERR, err)
  end
end
...
function LogglyLogHandler:log(conf)
  local serialize, err = get_serializer(conf.serializer or "default")
